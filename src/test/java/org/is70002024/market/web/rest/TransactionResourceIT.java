...
import static org.is70002024.market.domain.TransactionAsserts.*;
...
class TransactionResourceIT {
    ...
    public static Transaction createEntity() {
        return new Transaction().name(DEFAULT_NAME).transdate(DEFAULT_TRANSDATE).type(DEFAULT_TYPE).amount(DEFAULT_AMOUNT);
    }
    ...
    public static Transaction createUpdatedEntity() {
        return new Transaction().name(UPDATED_NAME).transdate(UPDATED_TRANSDATE).type(UPDATED_TYPE).amount(UPDATED_AMOUNT);
    }
    ...
    @Test
    @Transactional
    void checkTransdateIsRequired() throws Exception {
        long databaseSizeBeforeTest = getRepositoryCount();
        // set the field null
        transaction.setTransdate(null);
    ...
    void putExistingTransaction() throws Exception {
        ...
        updatedTransaction.name(UPDATED_NAME).transdate(UPDATED_TRANSDATE).type(UPDATED_TYPE).amount(UPDATED_AMOUNT);
    ...
    void partialUpdateTransactionWithPatch() throws Exception {
        ...
        partialUpdatedTransaction.name(UPDATED_NAME);
    ...
    void fullUpdateTransactionWithPatch() throws Exception {
        ...
        partialUpdatedTransaction.name(UPDATED_NAME).transdate(UPDATED_TRANSDATE).type(UPDATED_TYPE).amount(UPDATED_AMOUNT);
