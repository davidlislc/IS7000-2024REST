...
/**
 * REST controller for managing {@link org.is70002024.market.domain.Service}.
 */
@RestController
...
public class ServiceResource {
    ...
    public ResponseEntity<Service> createService(@Valid @RequestBody Service service) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/services/" + service.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, service.getId().toString()))
            .body(service);
    }
    ...
    public ResponseEntity<Service> partialUpdateService(
        ...
        Optional<Service> result = serviceRepository
            ...
            .map(existingService -> {
                ...
                if (service.getLevel() != null) {
                    existingService.setLevel(service.getLevel());
                }
                if (service.getInterval() != null) {
                    existingService.setInterval(service.getInterval());
                }
