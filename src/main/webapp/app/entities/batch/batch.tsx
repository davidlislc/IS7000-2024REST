            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('job')}>
                  Job <FontAwesomeIcon icon={getSortIconByFieldName('job')} />
                </th>
                <th className="hand" onClick={sort('rundate')}>
                  Rundate <FontAwesomeIcon icon={getSortIconByFieldName('rundate')} />
                </th>
                <th className="hand" onClick={sort('batchstatus')}>
                  Batchstatus <FontAwesomeIcon icon={getSortIconByFieldName('batchstatus')} />
                </th>
                <th>
            ...
            <tbody>
              {batchList.map((batch, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{batch.name}</td>
                  <td>{batch.job}</td>
                  <td>{batch.rundate ? <TextFormat type="date" value={batch.rundate} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  <td>{batch.batchstatus}</td>
                  <td>{batch.user ? batch.user.login : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/batch/${batch.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/batch/${batch.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
